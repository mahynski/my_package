...
3. Get coding! Follow the best practices below.  When you are ready to publish, proceed to step 4.
4. Bump the [version](https://semver.org/) in `my_package/__init__.py` and in `CITATION.cff`.
5. When finished, first enable preservation of this repo on [Zenodo](https://zenodo.org/), then create a release on GitHub. Zenodo will detect the new release and mint a DOI and badge. Update the `{github_id}` placeholder in this `README.md` and in `docs/index.rst` with the ID generated by Zenodo.
6. Follow [these instructions](https://packaging.python.org/en/latest/tutorials/packaging-projects/) to publish to PyPI.
7. Develop on a `dev` branch and merge into `main` for releases. This template assumes that pattern; adjust to taste.
